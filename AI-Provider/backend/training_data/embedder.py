...
class QueryExampleEmbedder:
    ...
    def _create_document(self, example: Dict[str, Any], domain: str) -> str:
        ...
        Uses FOCUSED text (primary questions only) for better semantic matching.
        Combining all variations dilutes the embedding and reduces match quality.
        ...
        Returns:
            Focused document text for embedding
        """
        # STRATEGY: Use only primary question(s) for embedding
        # The more focused the text, the better the semantic match
        # Variations are useful for metadata but hurt embedding quality

        question_uk = example.get("question_uk", "")
        question_en = example.get("question_en", "")

        # Prefer Ukrainian (most queries will be Ukrainian)
        # Add English as secondary for bilingual matching
        if question_uk and question_en:
            return f"{question_uk} | {question_en}"
        elif question_uk:
            return question_uk
        elif question_en:
            return question_en

        # Fallback: use first available variation
