"""
Multilingual Embedder for RAG System
...
from region_mapping import get_region_name, REGION_CODES


class MultilingualEmbedder:
    """Multilingual embeddings with Ukrainian support."""

    def __init__(self,
                 model_name: str = "paraphrase-multilingual-MiniLM-L12-v2",
                 chroma_dir: str = "chroma_db_multilingual",
                 collection_name: str = "concorddb_ukrainian_ml"):
    ...
    def enrich_document(self, doc: Dict[str, Any]) -> str:
        """
        Enrich document content with region names and better Ukrainian text.
        """
        content = doc.get("content", "")
        raw_data = doc.get("raw_data", {})
        table = doc.get("table", "")

        # Add region name if RegionID exists
        region_id = raw_data.get("RegionID")
        if region_id:
            # Look up region code from ID mapping
            # We need to query DB or use cached mapping
            pass

        # Enhance based on table type
        enrichments = []

        if "Client" in table:
            enrichments.append("Клієнт, замовник, покупець")
            name = raw_data.get("Name", "")
            if name:
                enrichments.append(f"Назва клієнта: {name}")

        elif "Product" in table:
            enrichments.append("Товар, продукт, продукція")
            name = raw_data.get("Name", "")
            if name:
                enrichments.append(f"Назва товару: {name}")

        elif "Order" in table:
            enrichments.append("Замовлення, заказ, продаж")

        elif "Sale" in table:
            enrichments.append("Продаж, реалізація, угода")

        elif "Debt" in table:
            enrichments.append("Борг, заборгованість, неоплачено")
            total = raw_data.get("Total")
            if total:
                enrichments.append(f"Сума боргу: {total} грн")

        elif "OrderItem" in table:
            enrichments.append("Позиція замовлення, товар у замовленні")

        # Combine enrichments with original content
        if enrichments:
            enriched = "\n".join(enrichments) + "\n\n" + content
            return enriched
    ...
    def embed_documents(self, documents: List[Dict[str, Any]],
        ...
        """Embed documents with multilingual model."""
        print("\n" + "="*60)
        print("EMBEDDING DOCUMENTS (MULTILINGUAL)")
        print("="*60 + "\n")
        ...
        for i in iterator:
            ...
            # Enrich and prepare texts
            texts = [self.enrich_document(doc)[:2000] for doc in batch]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Multilingual embedding for Ukrainian")
    parser.add_argument("--input", default="data/extracted_documents.json")
    parser.add_argument("--chroma-dir", default="chroma_db_multilingual")
    ...
    embedder = MultilingualEmbedder(chroma_dir=args.chroma_dir)
