...
class ProductAggregator:
    ...
    def __init__(
        ...
        logger.info(f"ProductAggregator initialized for {forecast_weeks} weeks")
    ...
    def aggregate_forecast(
        ...
        try:
            ...
            # 2. Allocate customers to weeks using probability distributions
            weekly_allocations = self._allocate_customers_to_weeks(
                predictions, weekly_buckets
            )

            # 3. Aggregate into weekly forecasts
            weekly_forecasts = self._aggregate_weekly_forecasts(
                weekly_allocations, weekly_buckets, price
            )

            # 4. Calculate summary metrics
            summary = self._calculate_summary(predictions, weekly_forecasts)

            # 5. Identify top customers by predicted volume
            ...
            # 7. Calculate model metadata
            metadata = self._calculate_metadata(predictions)

            # 8. Format weekly forecasts for API
            formatted_weeks = self._format_weekly_forecasts(weekly_forecasts)

            return ProductForecast(
                product_id=product_id,
                forecast_period_weeks=self.forecast_weeks,
                summary=summary,
                weekly_forecasts=formatted_weeks,
    ...
    def _format_weekly_forecasts(self, weekly_forecasts: List[WeeklyForecast]) -> List[Dict]:
        """
        Format weekly forecasts for API response
        """
        formatted = []
        ...
        for week in weekly_forecasts:
            formatted.append({
                ...
                'predicted_quantity': week.predicted_quantity,
                'predicted_revenue': week.predicted_revenue,
                'predicted_orders': week.predicted_orders,
