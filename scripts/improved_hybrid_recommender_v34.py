...
class ImprovedHybridRecommenderV34:
    """
    V3.4: Tunable Weights - Hyperparameter optimization
    - Co-purchase scoring (products bought together)
    - Purchase cycle detection (reorder timing)
    - Configurable feature weights for tuning

    Expected: +2-5% precision improvement over V3.3 through optimization
    """

    def __init__(self, conn=None, use_cache=True, custom_weights=None):
        if conn:
            self.conn = conn
        ...
        # V3.4: Configurable feature weights
        # Default weights (v3.3 baseline)
        ...
        logger.debug(f"Custom weights: {self.custom_weights if self.custom_weights else 'None (using defaults)'}")
    ...
    def get_recommendations_for_agreement(self, agreement_id: int, as_of_date: str, top_n: int = 20,
        ...
        # V3.4: Configurable weights (custom or default)
        # Use custom weights if provided, otherwise use segment-specific defaults
        if self.custom_weights:
            v3_weights = self.custom_weights
        else:
            # Default V3.3 weights by segment
            if segment == "HEAVY":
                v3_weights = self.default_weights
        ...
        for product_id in all_repurchase_products:
            ...
            # V3.4 FORMULA: Weighted sum with configurable weights
